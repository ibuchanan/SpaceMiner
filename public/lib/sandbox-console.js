...
var Sandbox = {
    ...
    events: function(eventSpec) {
      if (eventSpec.commandSent) {
        this.set({commandSent: eventSpec.commandSent});
      }
    },
    ...
    // TODO: we can use this logic to hook into this event when we want to take it this far
    /*
      var commandSent = this.get('commandSent');
      if (commandSent) {
        var inspectResult = commandSent(command);        
        if (inspectResult && !inspectResult.correct) {
          this.log("Oops! Try again. That wasn't what this step expected you to type.");
          this.log('You typed: ' + command);
          return;
        }
      }    
    */
	...
	View : Backbone.View.extend({
		...
		keydown: function(e) {
			...
			if ( ([16,17,18]).indexOf(e.which, true) > -1 ) this.ctrl = true;
        ...
        console.log("The val is:" + val);
          ...
          console.log("Current hist:" + this.currentHistory);
					this.update();
					return false;
				}
		...
		keyup: function(e) {
			...
			if ( ([16,17,18]).indexOf(e.which, true) > -1 ) this.ctrl = false;
		},
