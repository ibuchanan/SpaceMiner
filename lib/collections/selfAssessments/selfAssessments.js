...
SelfAssessments.findByResourcePath = function(resourcePath, criteria, options) {
  criteria = criteria || {};
  options = options || {};
  if (resourcePath) criteria.resourcePath = {$regex: resourcePath};
...
SelfAssessments.helpRequestCancel = function(resourcePath, userId) {
  userId = userId || Meteor.userId();
  var assessment = findByUserIdAndResourcePath(userId, resourcePath);
  if (assessment) {
    var updates = defaults();
...
SelfAssessments.helpAvailableCancel = function(resourcePath, userId) {
  // TODO update only most recent
  userId = userId || Meteor.userId();
  var assessment = findByUserIdAndResourcePath(userId, resourcePath);
  if (assessment) {
...
SelfAssessments.wantsHelp = function(resourcePath, userId) {
  userId = userId || Meteor.userId();
  var assessment = SelfAssessments.findByUserIdAndResourcePath(userId, resourcePath);
  return assessment.helpRequested === true;
};

SelfAssessments.willHelp = function(resourcePath, userId) {
  userId = userId || Meteor.userId();
  var assessment = SelfAssessments.findByUserIdAndResourcePath(userId, resourcePath);
  return assessment.helpMadeAvailable === true;
};