...
// print( true && 55 >= 54 ) // Step 1: JavaScript evaluates 5 > 3 to true
// print( true && true ) // Step 2: JavaScript evaluates 55 >= 54 to true`)
)
        ]
      });
...
+ code(`var rainbow = ["Red", "Orange", "Yellow", "Green", "Blue", "Indigo", "Violet"]`) +
      `${dbr}
And, we know we can print all these values in a simple, but repetitive way:
...
`${dbr}Is Approach #1 or Approach #2 easier to type? 
${dbr}
...
`, true) +          
`${dbr} Pretty cool, right? The code <code>print</code>, <code>printArray</code> and <code>printInColor</code> are all <b>functions</b>, and <b>functions</b> make life a lot easier and less repetitive!`),
      sec('Functions',
...
`${dbr} This is the basic format for a function. You setup the function with the keyword <i>function</i> followed by a word that represents the name for the function/ name for your function. A keyword is a word that is reserved for a specific purpose and cannot be changed. That is, the word function can only be used to create a function.`,
