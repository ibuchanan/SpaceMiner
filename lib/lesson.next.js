...
function program(id, script, useStringify) {
  if (useStringify === undefined) useStringify = true;
  var data = JSON.stringify({
    useStringify: useStringify,
    script: script
  });
...
`;
  ...
  that.Lesson.defaultLessons.push(
    {
    _id: 'flowControl',
    title: 'Use flow control statements ...... TODO',
    sections: [
      sec('Computers need you',
           `Here's a very brief computer program written in the <b>JavaScript</b> programming language. JavaScript is built-in to all web modern web browsers that run on desktops, laptops, phones, and tablet devices:
${dbr}` 
          + code('print("Hello, world!")') +
`${dbr}Study this code for a moment before continuing. What do you think it's going to do just by looking at it?
`,
...
<i>Try these one by one first as explained above, but you can come back later and execute or reset all of them at once with these buttons:</i> ` + sampleProgramExecAll() + dbr
          + association([
            ...
            '[1, 2, 3]',
            'An Array that contains only Number values'
          ])
          + programSort(`var x;
x = 1;
alert(x);
